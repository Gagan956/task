...
const checkInMongoDB = async (todo_id: string): Promise<boolean> => {
  const existing = await Todo.findOne({ todo_id });
  return !!existing;
};

// Check if todo exists in Redis
const checkInRedis = async (todo_id: string): Promise<boolean> => {
...
const fetchTodosFromAirtable = async () => {
  ...
  const todos = await Promise.all(
    records.map(async (record) => {
      const todo_id = record.fields.todo_id as string || record.id;
      const todo_name = record.fields.todo_name as string || 'Untitled';
      const created_at = record.fields.created_at as string || new Date().toISOString();
...
const moveTodoToRedis = async ({ airtableRecordId, todo_name }: TodoPayload) => {
...
const moveTodoToDatabase = async ({ airtableRecordId }: { airtableRecordId: string }) => {
  const key = `todo:${airtableRecordId}`;
  
  // Check if todo exists in Redis
  const exists = await redisClient.exists(key);
  if (!exists) throw new Error('Todo not found in Redis');

  // Check if already exists in MongoDB
  const alreadyInDb = await checkInMongoDB(airtableRecordId);
  if (alreadyInDb) throw new Error('Todo already exists in MongoDB');

  // Fetch todo data from Redis
  const data = await redisClient.get(key);
  if (!data) throw new Error('Failed to retrieve todo data from Redis');
